...
if not SECRET_KEY:
    # EN: Refuse to start rather than silently signing tokens with a public, guessable key.
    # JP: 公開された推測可能な鍵でトークンに署名するより、起動を拒否します。
    raise RuntimeError(
        "SECRET_KEY environment variable is not set. Add it to backend/.env"
    )
