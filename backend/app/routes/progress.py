...
@router.get("/", response_model=list[schemas.ProgressSnapshotResponse])
def get_progress_snapshots_for_current_user(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    # EN: Get all progress snapshots for the currently logged-in user.
    # JP: 現在ログイン中のユーザーのすべての進捗スナップショットを取得します。
    return (
        db.query(models.ProgressSnapshot)
        .filter(models.ProgressSnapshot.user_id == current_user.id)
        .all()
    )


@router.put("/{snapshot_id}", response_model=schemas.ProgressSnapshotResponse)
def update_progress_snapshot(snapshot_id: int, snapshot_update: schemas.ProgressSnapshotUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    # EN: Make sure users can only update their own progress snapshots.
    # JP: ユーザーが自分自身の進捗スナップショットだけを更新できるようにします。
    ensure_user_owns_resource(snapshot.user_id, current_user.id)
...
def delete_progress_snapshot(snapshot_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    # EN: Make sure users can only delete their own progress snapshots.
    # JP: ユーザーが自分自身の進捗スナップショットだけを削除できるようにします。
    ensure_user_owns_resource(snapshot.user_id, current_user.id)
